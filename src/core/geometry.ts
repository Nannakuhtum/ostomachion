 ...
 * Flip is applied before rotation: v → rot(flip(v)) + t.
 */
 ...
 * order, as produced by transforming a ring vertex-by-vertex). Returns null
 * if no flip/rotation/translation matches.
 */
export function poseForRing(base: Ring, target: Ring): Pose | null {
  for (const flipped of [false, true]) {
    for (const rot of [0, 1, 2, 3] as const) {
      const c = transformRing(base, { flipped, rot, tx: 0, ty: 0 });
